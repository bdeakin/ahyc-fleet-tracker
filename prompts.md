...
- Default chart: **NOAA chart (paper style)** — ENC via the Maritime Chart Service with paper symbols, plain boundaries, four depth shades (12 / 30 / 60 ft), soundings in feet. NOAA ENC tiles are blank below zoom 8, so Esri Ocean stays underneath and NOAA only paints from there in. Carto Voyager harbor layer (`CARTO_API_KEY` → keyed tiles) + OpenSeaMap seamarks (zoom ≥ 14 only), Esri Ocean regional overview, and the all-detail ENC display are the other options
