...
10d. Club boats are marked with the AHYC burgee (`apps/web/src/burgee.ts`), an SVG string shared by the Leaflet `divIcon` markers and the `BurgeeGlyph` component used in the legend, search results, and detail-pane heading. The marker offset is derived from each marker's own height so the flag clears both the moving arrow and the stopped circle.
